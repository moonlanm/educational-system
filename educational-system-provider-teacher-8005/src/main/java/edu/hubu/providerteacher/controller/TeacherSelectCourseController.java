...
public class TeacherSelectCourseController {
    ...
    public CommonResult selectAll(@RequestParam(defaultValue = "teacherSelectCourseId") String orderBy) {
        List<TeacherSelectCourse> list = service.selectAllOrderBy(orderBy);
    ...
    public CommonResult selectAllPage(
        ...
        List<TeacherSelectCourse> list = service.selectAllPageOrderBy(currentPage, pageSize, orderBy);
        return new CommonResult().setCode(200).setCurrentPage(1)
            .setData(list)
            .setMessage("SUCCESS")
            .setTotal(list.size());
    ...
    @ApiImplicitParams(value = {
            ...
            @ApiImplicitParam(name = "pageSize", value = "页面大小，默认为5,非必须参数", dataTypeClass = Integer.class),
            @ApiImplicitParam(name = "orderBy", value = "排序字段，默认为老师选课ID，非必须参数", dataTypeClass = String.class),
            @ApiImplicitParam(name = "teacherSelectCourse", value = "查询老师选课的条件", required = true, dataTypeClass = edu.hubu.commons.mybatis.model.TeacherSelectCourse.class)
    })
    public CommonResult selectLikePage(
        ...
        List<edu.hubu.commons.mybatis.model.TeacherSelectCourse> list = service.selectLikePageOrderBy(currentPage, pageSize,
            orderBy, teacherSelectCourse);
        return new CommonResult().setCode(200).setCurrentPage(1)
            .setData(list)
            .setMessage("SUCCESS")
            .setTotal(list.size());
    ...
    public CommonResult save(@RequestBody TeacherSelectCourse teacherSelectCourse) {
        return new CommonResult().setMessage("SUCCESS").setData(service.save(teacherSelectCourse)).setMessage("SUCCESS")
