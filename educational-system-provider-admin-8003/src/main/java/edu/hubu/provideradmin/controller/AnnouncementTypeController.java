...
public class AnnouncementTypeController {
    ...
    public CommonResult selectAll(@RequestParam(defaultValue = "announcementTypeId") String orderBy) {
        List<AnnouncementType> list = service.selectAllOrderBy(orderBy);
    ...
    public CommonResult selectAllPage(
        ...
        List<AnnouncementType> list = service.selectAllPageOrderBy(currentPage, pageSize, orderBy);
        return new CommonResult().setCode(200).setCurrentPage(1)
            .setData(list)
            .setMessage("SUCCESS")
            .setTotal(list.size());
    ...
    @ApiImplicitParams(value = {
            ...
            @ApiImplicitParam(name = "pageSize", value = "页面大小，默认为5,非必须参数", dataTypeClass = Integer.class),
            @ApiImplicitParam(name = "orderBy", value = "排序字段，默认为公告类型ID，非必须参数", dataTypeClass = String.class),
            @ApiImplicitParam(name = "announcementType", value = "查询公告类型的条件", required = true, dataTypeClass = edu.hubu.commons.mybatis.model.AnnouncementType.class)
    })
    public CommonResult selectLikePage(
        ...
        List<edu.hubu.commons.mybatis.model.AnnouncementType> list = service.selectLikePageOrderBy(currentPage, pageSize,
            orderBy, announcementType);
        return new CommonResult().setCode(200).setCurrentPage(1)
            .setData(list)
            .setMessage("SUCCESS")
            .setTotal(list.size());
    ...
    public CommonResult save(@RequestBody AnnouncementType announcementType) {
        return new CommonResult().setMessage("SUCCESS").setData(service.save(announcementType)).setMessage("SUCCESS")
