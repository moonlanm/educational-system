...
public class MajorController {
    ...
    public CommonResult selectAll(@RequestParam(defaultValue = "majorId") String orderBy) {
        List<Major> list = service.selectAllOrderBy(orderBy);
    ...
    public CommonResult selectAllPage(
        ...
        List<Major> list = service.selectAllPageOrderBy(currentPage, pageSize, orderBy);
        return new CommonResult().setCode(200).setCurrentPage(1)
            .setData(list)
            .setMessage("SUCCESS")
            .setTotal(list.size());
    ...
    @ApiImplicitParams(value = {
            ...
            @ApiImplicitParam(name = "pageSize", value = "页面大小，默认为5,非必须参数", dataTypeClass = Integer.class),
            @ApiImplicitParam(name = "orderBy", value = "排序字段，默认为专业ID，非必须参数", dataTypeClass = String.class),
            @ApiImplicitParam(name = "major", value = "查询专业的条件", required = true, dataTypeClass = edu.hubu.commons.mybatis.model.Major.class)
    })
    public CommonResult selectLikePage(
        ...
        List<edu.hubu.commons.mybatis.model.Major> list = service.selectLikePageOrderBy(currentPage, pageSize,
            orderBy, major);
        return new CommonResult().setCode(200).setCurrentPage(1)
            .setData(list)
            .setMessage("SUCCESS")
    ...
    public CommonResult save(@RequestBody Major major) {
        return new CommonResult().setMessage("SUCCESS").setData(service.save(major)).setMessage("SUCCESS")
