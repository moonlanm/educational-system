...
import edu.hubu.consumeradmin.service.IRolePermissionMapService;
...
@RestController
@RequestMapping("/consumer/rolePermissionMap")
@Api(tags = "角色权限映射api")
public class RolePermissionMapController {
    ...
    @ApiImplicitParams(value = {
            ...
            @ApiImplicitParam(name = "pageSize", value = "页面大小，默认为5,非必须参数", dataTypeClass = Integer.class),
            @ApiImplicitParam(name = "orderBy", value = "排序字段，默认为角色权限映射ID，非必须参数", dataTypeClass = String.class),
            @ApiImplicitParam(name = "rolePermissionMap", value = "查询角色权限映射的条件", required = true, dataTypeClass =
                    edu.hubu.commons.mybatis.model.RolePermissionMap.class)
    })
    public CommonResult selectLikePage(
        ...
        return service.selectLikePage(currentPage, pageSize, orderBy, rolePermissionMap);
    }
    ...
    public CommonResult save(@RequestBody RolePermissionMap rolePermissionMap) {
        return service.save(rolePermissionMap);
    }
    ...
    public CommonResult update(@PathVariable("id") String id, @RequestBody RolePermissionMap rolePermissionMap) {
        return service.update(id, rolePermissionMap);
    }
